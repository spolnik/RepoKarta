...
func (s *shardedSearcher) replace(shards map[string]zoekt.Searcher) {
	...
	for key, shard := range shards {
		...
		if old != nil && old.Searcher != nil {
			if s.closing && runtime.GOOS == "windows" {
				// Windows does not permit replacing or removing a file while a
				// mapped view remains open. Close synchronously during an explicit
				// searcher shutdown instead of waiting for a GC finalizer.
				old.Close()
				continue
			}
			//                 _ ___                /^^\ /^\  /^^\_
